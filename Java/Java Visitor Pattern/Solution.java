...
class ProductOfRedNodesVisitor extends TreeVis {
    private final int computedM = 1000000007;
    ...
    public void visitNode(TreeNode node) {
        if (node.getColor().equals(Color.RED)) {
            result = (result * node.getValue()) % computedM;
        }
    }

    public void visitLeaf(TreeLeaf leaf) {
        if (leaf.getColor().equals(Color.RED)) {
            result = (result * leaf.getValue()) % computedM;
        }
    }
}

class FancyVisitor extends TreeVis {
    private int nonLeavesEvenDepthSum = 0;
    private int greenLeafSum = 0;

    public int getResult() {
        return Math.abs(nonLeavesEvenDepthSum - greenLeafSum);
    }

    public void visitNode(TreeNode node) {
        if (node.getDepth() % 2 == 0) {
            nonLeavesEvenDepthSum += node.getValue();
        }
    }
...
public class Solution {
    ...
    private static void createEdges(int u, int v) {
